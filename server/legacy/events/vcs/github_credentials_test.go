...
func TestGithubClient_GetUser_AppSlug(t *testing.T) {
	...
	anonClient, err := vcs.NewGithubClient(testServer, anonCreds, logging.NewNoopCtxLogger(t), &testAllocator{}, mergeabilityChecker)
	Ok(t, err)
...
func TestGithubClient_AppAuthentication(t *testing.T) {
	...
	_, err = vcs.NewGithubClient(testServer, appCreds, logging.NewNoopCtxLogger(t), &testAllocator{}, mergeabilityChecker)
	Ok(t, err)
