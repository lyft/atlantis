...
func TestGithubClient_GetModifiedFiles(t *testing.T) {
	...
	respTemplate := `[
  {
	...
	client, err := vcs.NewGithubClient(testServerURL.Host, &vcs.GithubUserCredentials{"user", "pass"}, logger, &testAllocator{}, mergeabilityChecker)
	Ok(t, err)
...
func TestGithubClient_MarkdownPullLink(t *testing.T) {
	mergeabilityChecker := vcs.NewPullMergeabilityChecker("atlantis")
	client, err := vcs.NewGithubClient("hostname", &vcs.GithubUserCredentials{"user", "pass"}, logging.NewNoopCtxLogger(t), &testAllocator{}, mergeabilityChecker)
	Ok(t, err)
