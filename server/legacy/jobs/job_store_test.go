...
import (
	...
	"github.com/runatlantis/atlantis/server/legacy/jobs/mocks"
	"github.com/runatlantis/atlantis/server/legacy/jobs/mocks/matchers"
...
func TestJobStore_Get(t *testing.T) {
	...
	t.Run("load from storage backend when not in memory", func(t *testing.T) {
		...
		When(storageBackend.Read(matchers.AnyContextContext(), AnyString())).ThenReturn(expectedLogs, nil)
	...
	t.Run("error when reading from storage backend fails", func(t *testing.T) {
		...
		When(storageBackend.Read(matchers.AnyContextContext(), AnyString())).ThenReturn([]string{}, errors.New("error"))
