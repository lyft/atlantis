...
func (r *EnvStepRunner) Run(ctx context.Context, prjCtx command.ProjectContext, command string, value string, path string, envs map[string]string) (string, error) {
	if value != "" {
		return value, nil
	}
	res, err := r.RunStepRunner.Run(ctx, prjCtx, command, path, envs)
