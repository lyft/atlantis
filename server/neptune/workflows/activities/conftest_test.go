	...
	for _, c := range cases {
		t.Run("request param takes precedence", func(t *testing.T) {
			...
			testClient := &testTfClient{
				...
				jobID:         jobID,
				path:          path,
				cmd:           command.NewSubCommand(command.ConftestTest).WithDupArgs(c.ExpectedArgs...).WithInput("some/path/output.json").WithFlags(c.ExpectedFlags...),
...
func TestConftest_StreamsOutput(t *testing.T) {
	...
	testClient := &testTfClient{
		...
		jobID:         jobID,
		path:          path,
		cmd:           command.NewSubCommand(command.ConftestTest).WithDupArgs(expectedArgs...).WithInput("some/path/output.json").WithFlags(expectedFlags...),
