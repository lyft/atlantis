...
func (e *EnvStepRunner) Run(ctx workflow.Context, step steps.Step, jobContext deploy.JobContext, envs map[string]string) (string, error) {
	if step.EnvVarValue != "" {
		return step.EnvVarValue, nil
	}

	res, err := e.RunStepRunner.Run(ctx, step, jobContext, envs)
