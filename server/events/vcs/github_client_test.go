...
func TestGithubClient_GetModifiedFilesMovedFile(t *testing.T) {
	resp := `[
  {
	...
	client, err := vcs.NewGithubClient(testServerURL.Host, &vcs.GithubUserCredentials{"user", "pass"}, logging.NewNoopCtxLogger(t), mergeabilityChecker, internalClient, &mockStatusUpdater{})
	Ok(t, err)
	defer disableSSLVerification()()
...
func TestGithubClient_MarkdownPullLink(t *testing.T) {
	...
	assert.NoError(t, err)
	client, err := vcs.NewGithubClient("hostname", &vcs.GithubUserCredentials{"user", "pass"}, logging.NewNoopCtxLogger(t), mergeabilityChecker, internalClient, &mockStatusUpdater{})
	Ok(t, err)
