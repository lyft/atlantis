...
func TestNewGithubClient_GithubCom(t *testing.T) {
	...
	client, err = NewGithubClient("github.com", &GithubUserCredentials{"user", "pass"}, logging.NewNoopCtxLogger(t), mergeabilityChecker, internalClient, &mockStatusUpdater{})
	Ok(t, err)
...
func TestNewGithubClient_NonGithub(t *testing.T) {
	...
	client, err := NewGithubClient("example.com", &GithubUserCredentials{"user", "pass"}, logging.NewNoopCtxLogger(t), mergeabilityChecker, internalClient, &mockStatusUpdater{})
	Ok(t, err)
