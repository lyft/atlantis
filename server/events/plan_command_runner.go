...
func (p *PlanCommandRunner) runAutoplan(ctx *command.Context) {
	...
	if err != nil {
		if statusErr := p.commitStatusUpdater.UpdateCombined(context.TODO(), baseRepo, pull, models.FailedCommitStatus, command.Plan, ctx.StatusID); statusErr != nil {
	...
	if len(projectCmds) == 0 {
		...
		if err := p.commitStatusUpdater.UpdateCombinedCount(context.TODO(), baseRepo, pull, models.SuccessCommitStatus, command.Plan, 0, 0, ctx.StatusID); err != nil {
			ctx.Log.Warnf("unable to update commit status: %s", err)
		}
		if err := p.commitStatusUpdater.UpdateCombinedCount(context.TODO(), baseRepo, pull, models.SuccessCommitStatus, command.PolicyCheck, 0, 0, ctx.StatusID); err != nil {
			ctx.Log.Warnf("unable to update commit status: %s", err)
		}
		if err := p.commitStatusUpdater.UpdateCombinedCount(context.TODO(), baseRepo, pull, models.SuccessCommitStatus, command.Apply, 0, 0, ctx.StatusID); err != nil {
			ctx.Log.Warnf("unable to update commit status: %s", err)
		}
		return
	}
...
func (p *PlanCommandRunner) run(ctx *command.Context, cmd *command.Comment) {
	...
	if err = p.commitStatusUpdater.UpdateCombined(context.TODO(), baseRepo, pull, models.PendingCommitStatus, command.Plan, ctx.StatusID); err != nil {
		ctx.Log.Warnf("unable to update commit status: %s", err)
	}

	projectCmds, err := p.prjCmdBuilder.BuildPlanCommands(ctx, cmd)
	if err != nil {
		if statusErr := p.commitStatusUpdater.UpdateCombined(context.TODO(), ctx.Pull.BaseRepo, ctx.Pull, models.FailedCommitStatus, command.Plan, ctx.StatusID); statusErr != nil {
			ctx.Log.Warnf("unable to update commit status: %s", statusErr)
		}
