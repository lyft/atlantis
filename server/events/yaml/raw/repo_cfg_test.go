...
func TestConfig_ToValid(t *testing.T) {
	cases := []struct {
		...
		{
			description: "automerge, parallel_apply, and delete_all_projects true",
			input: raw.RepoCfg{
				Version:           Int(2),
				Automerge:         Bool(true),
				ParallelApply:     Bool(true),
				DeleteAllProjects: Bool(true),
			},
			exp: valid.RepoCfg{
				Version:           2,
				Automerge:         true,
		...
		{
			description: "automerge, parallel_apply, and delete_all_projects false",
			input: raw.RepoCfg{
				Version:       Int(2),
