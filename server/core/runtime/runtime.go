...
type AsyncTFExec interface {
	...
	RunCommandAsync(ctx project.Context, path string, args []string, envs map[string]string, v *version.Version, workspace string) <-chan terraform.Line
	RunCommandAsyncWithInput(ctx project.Context, path string, args []string, envs map[string]string, v *version.Version, workspace string, input <-chan string) <-chan terraform.Line
}
